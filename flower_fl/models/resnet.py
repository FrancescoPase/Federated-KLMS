...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, option='A', device=None):
        ...
        num_groups = 2
        self.gn1 = nn.GroupNorm(num_groups=num_groups, num_channels=planes, device=device)
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=1, padding=1, bias=False, device=device)

        self.gn2 = nn.GroupNorm(num_groups=num_groups, num_channels=planes, device=device)
            ...
            if option == 'A':
                ...
                self.shortcut = nn.Sequential(
                     nn.Conv2d(in_planes, self.expansion * planes, kernel_size=1, stride=stride, bias=False,
                                  device=device),
                     nn.GroupNorm(num_groups=2, num_channels=self.expansion * planes, device=device)
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, device=None):
        ...
        self.layer1 = self._make_layer(block, 16, num_blocks[0], stride=1, device=device)
        self.layer2 = self._make_layer(block, 32, num_blocks[1], stride=2, device=device)
        self.layer3 = self._make_layer(block, 64, num_blocks[2], stride=2, device=device)
