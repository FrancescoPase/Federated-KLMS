...
class Mask2CNN(nn.Module):
    """
            2Conv model studied in
            https://proceedings.neurips.cc/paper/2019/file/1113d7a76ffceca1bb350bfe145467c6-Paper.pdf for cifar10.
    """

    def __init__(self, init='ME_init', activation='relu', device=None):
        super(Mask2CNN, self).__init__()
        self.init = init
        self.activation = activation
        self.conv1 = MaskedConv2d(3, 64, kernel_size=3, stride=1, padding='same', init=init, device=device)
        self.conv2 = MaskedConv2d(64, 64, kernel_size=3, stride=1, padding='same', init=init, device=device)

        self.dense1 = MaskedLinear(16384, 256, init=init, device=device)
