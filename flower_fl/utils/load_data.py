...
def get_default_data_transforms(dataset, train=True, verbose=False):
    ...
    if 'mnist' in dataset:
        transforms_train = {
            'general': transforms.Compose([transforms.ToTensor(), transforms.Normalize(*[0.1307, 0.3081])
                                           ])
        }
        transforms_eval = {
            'general': transforms.Compose([transforms.ToTensor(), transforms.Normalize(*[0.1307, 0.3081])
...
def get_data_loaders(dataset, nclients, batch_size, classes_pc=10, split='iid', real_wd=False):
    ...
    if split == 'iid':
        ...
        data_loader_test = DataLoader(data_test, batch_size=len(data_test), shuffle=True)
